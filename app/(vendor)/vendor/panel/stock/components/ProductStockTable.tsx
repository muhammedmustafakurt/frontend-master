...
export default function ProductStockTable({
    ...
    const getStockBadge = (stock: number) => {
        if (stock === 0) {
            return <span className="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">Tükendi</span>;
        } else if (stock < 10) {
            return <span className="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Düşük</span>;
        } else {
            return <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Normal</span>;
        }
    };
    ...
    const handleStockSave = (productId: number) => {
        if (tempStock >= 0) {
            onStockUpdate(productId, tempStock);
        }
    ...
    const handlePriceSave = (productId: number) => {
        if (tempPrice >= 0) {
            onPriceUpdate(productId, tempPrice);
        }
